...
import { getTodayOraculo, getRecentOraculoDays, saveOraculoResult, getOraculoPdfUrl, saveOraculoDriveLink } from '@/lib/supabase';
import { generateMorningBrief, renderNewspaperHtml, extractProfessionalBriefFromImage } from '@/lib/oraculo';
import { renderOraculoPdf } from '@/lib/oraculoPdf';
import { uploadOraculoPdfToDrive } from '@/lib/googleDrive';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const pdf = await renderOraculoPdf(day.brief_date, itinerary);

    const updated = await saveOraculoResult(userEmail, professionalBrief, inputType, itinerary, html, pdf);
    const pdfUrl = updated.pdf_path ? await getOraculoPdfUrl(updated.pdf_path) : null;

    // Best-effort — a missing/expired Drive grant should never break the periódico itself.
    let driveLink: string | null = null;
    try {
      const drive = await uploadOraculoPdfToDrive(userEmail, day.brief_date, pdf);
    ...
    return NextResponse.json({ day: { ...updated, pdf_drive_link: driveLink }, pdfUrl, driveLink });
  } catch (err) {
