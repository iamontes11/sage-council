...
import { ChevronRight, ChevronLeft, SkipForward, RotateCcw, Send, CheckCircle2, Zap } from 'lucide-react';
...
export function ADHDSequenceHelper() {
  ...
  const sendToCouncil = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/chats', {
        ...
        body: JSON.stringify({ title: `ADHD: ${step.name}` }),
      });
      const { chat } = await res.json();
      router.push(`/chat/${chat.id}?starter=${encodeURIComponent(step.prompt(task))}`);
    } catch {
      setSending(false);
    }
  }, [step, task, router, sending]);

  const completedCount = STEPS.length - skipped.size;
  ...
  if (phase === 'input') {
    return (
      <motion.div
        ...
        className="flex-1 flex flex-col items-center justify-center px-6 py-12 max-w-xl mx-auto w-full"
      >
        <div className="text-center space-y-3 mb-10">
          <div className="text-5xl mb-4">⚡</div>
          <h1 className="text-2xl font-bold text-white">ADHD Sequence Helper</h1>
          <p className="text-neutral-400 text-sm leading-relaxed">
            Guía de 7 pasos para ejecutar cualquier tarea sin parálisis.
            El Consejo te acompañará en cada uno.
          </p>
        </div>
        ...
        <div className="w-full space-y-4">
          <div className="bg-[#18181b] border border-white/[0.10] focus-within:border-violet-500/45 rounded-2xl px-4 py-3 transition-all duration-200">
            ...
            <textarea
              ...
              onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); startSequence(); } }}
        ...
        <div className="mt-10 w-full space-y-2">
          <p className="text-[11px] text-neutral-600 uppercase tracking-wider font-semibold text-center mb-4">Los 7 pasos</p>
          {STEPS.map((s) => (
            <div key={s.key} className="flex items-center gap-3 px-3 py-2 rounded-xl text-sm text-neutral-500">
              <span className="text-base shrink-0">{s.emoji}</span>
              <span className="flex-1">{s.name}</span>
              {s.optional && (
                <span className="text-[10px] text-neutral-700 bg-white/5 px-2 py-0.5 rounded-full">Opcional</span>
              )}
            </div>
          ))}
        </div>
  ...
  if (phase === 'done') {
    return (
      <motion.div
        ...
        className="flex-1 flex flex-col items-center justify-center px-6 py-12 max-w-xl mx-auto w-full text-center"
      >
        <div className="text-6xl mb-6">✅</div>
        <h2 className="text-2xl font-bold text-white mb-3">¡Secuencia Completada!</h2>
        <p className="text-neutral-400 text-sm mb-2">
          Tarea: <span className="text-neutral-200 font-medium">{task}</span>
        </p>
        <p className="text-neutral-500 text-sm mb-10">
          {completedCount} de 7 pasos completados · ~20-25 minutos
        </p>

        <div className="w-full space-y-2 mb-10">
          {STEPS.map((s) => (
            <div key={s.key} className={`flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm ${
              ...
              {skipped.has(s.key) && <span className="text-[10px] text-neutral-700 ml-auto">Saltado</span>}
            </div>
          ))}
        </div>
        ...
        <div className="w-full space-y-3">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 bg-white/5 hover:bg-white/10 text-neutral-300 font-medium py-3 rounded-xl transition-colors text-sm"
          >
            Ir al Consejo
          </button>
        </div>
          ...
          {STEPS.map((s, i) => (
            <div
              ...
              className={`h-1 flex-1 rounded-full transition-colors duration-300 ${
                i < currentStep
                  ? skipped.has(s.key) ? 'bg-white/10' : 'bg-violet-600'
                  : i === currentStep
                  ? 'bg-violet-400'
                  : 'bg-white/5'
              }`}
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <div className="flex items-start gap-4 mb-5">
            <div className="w-14 h-14 rounded-2xl bg-violet-900/30 border border-violet-500/20 flex items-center justify-center text-2xl shrink-0">
              {step.emoji}
            </div>
          ...
          {/* Prompt */}
          <div className="flex-1 bg-[#13121a] border border-white/[0.08] rounded-2xl p-5 mb-5">
            <p className="text-[11px] text-violet-400/70 uppercase tracking-wider font-semibold mb-3">
              Prompt para el Consejo
            </p>
          ...
          <button
            ...
            disabled={sending}
            className="w-full flex items-center justify-center gap-2.5 bg-violet-600 hover:bg-violet-500 disabled:opacity-50 disabled:cursor-not-allowed text-white font-semibold py-3.5 rounded-xl transition-colors text-sm mb-3"
          >
            {sending ? (
              <motion.div
                animate={{ rotate: 360 }}
